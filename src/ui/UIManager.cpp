    ...
    if (activeSatIndex < satellites.size())
    {
        ...
        if (leftSidebarOffset > -(float)UITheme::SIDEBAR_WIDTH + 10.0f)
        {
            drawLeftSidebar(fonts, satellites, activeSatIndex, sunDirection,
                            groundStations, accessStats, forceModel);
        }
    ...
    int fps)
{
    ...
    Color fpsColor = (fps >= 55) ? UITheme::ACCENT : (fps >= 30) ? UITheme::WARNING
                                                                 : UITheme::DANGER;
    ...
    const ForceModel &forceModel)
{
        ...
        mousePos.y >= y && mousePos.y <= y + height)
    {
        ...
        if (maxScroll < 0)
            maxScroll = 0;
        if (leftSidebarScroll < 0)
            leftSidebarScroll = 0;
        if (leftSidebarScroll > maxScroll)
            leftSidebarScroll = maxScroll;
    }
    ...
    drawForceModelPanel(fonts, forceModel, contentX, yOffset, contentWidth, yOffset);
    ...
    if (maxScroll > 0)
    {
        ...
        if (thumbHeight < 30)
            thumbHeight = 30; // Minimum thumb size
    ...
    int x, int y, int width, int &yOffset)
{
    ...
    {
        Color textColor = satellites[i].isVisible() ? satellites[i].getStats().familyColor : UITheme::TEXT_MUTED;
    ...
    int x, int y, int width, int &yOffset)
{
    ...
    Color betaColor = (fabs(solar.betaAngle) < 30.0) ? UITheme::ACCENT : (fabs(solar.betaAngle) < 60.0) ? UITheme::WARNING
                                                                                                        : UITheme::DANGER;
...
void UIManager::drawForceModelPanel(
    const FontSystem &fonts,
    const ForceModel &forceModel,
    int x, int y, int width, int &yOffset)
{
    ...
    // Section header
    fonts.drawText("FORCE MODEL", x, yOffset, UITheme::FONT_SIZE_H2,
                   UITheme::INFO, true);
    yOffset += 28;
    ...
    // Point mass (always on)
    fonts.drawText("Point Mass Gravity", x, yOffset, UITheme::FONT_SIZE_BODY,
                   UITheme::TEXT_SECONDARY);
    fonts.drawText("[ON]", x + 200, yOffset, UITheme::FONT_SIZE_BODY,
                   UITheme::ACCENT, true);
    yOffset += 20;

    // J2 perturbation
    fonts.drawText("J2 (Oblateness)", x, yOffset, UITheme::FONT_SIZE_BODY,
                   UITheme::TEXT_SECONDARY);
    Color j2Color = forceModel.j2Perturbation ? UITheme::ACCENT : UITheme::TEXT_MUTED;
    const char *j2Status = forceModel.j2Perturbation ? "[ON]" : "[OFF]";
    fonts.drawText(j2Status, x + 200, yOffset, UITheme::FONT_SIZE_BODY,
                   j2Color, forceModel.j2Perturbation);
    yOffset += 20;

    // Future forces (grayed out)
    fonts.drawText("Atmospheric Drag", x, yOffset, UITheme::FONT_SIZE_BODY,
                   UITheme::TEXT_MUTED);
    fonts.drawText("[Future]", x + 200, yOffset, UITheme::FONT_SIZE_SMALL,
                   UITheme::TEXT_MUTED);
    yOffset += 20;

    fonts.drawText("Solar Radiation", x, yOffset, UITheme::FONT_SIZE_BODY,
                   UITheme::TEXT_MUTED);
    fonts.drawText("[Future]", x + 200, yOffset, UITheme::FONT_SIZE_SMALL,
                   UITheme::TEXT_MUTED);
    yOffset += 20;

    fonts.drawText("Third Body (Moon)", x, yOffset, UITheme::FONT_SIZE_BODY,
                   UITheme::TEXT_MUTED);
    fonts.drawText("[Future]", x + 200, yOffset, UITheme::FONT_SIZE_SMALL,
                   UITheme::TEXT_MUTED);
    yOffset += 20;
    ...
    // Info note
    yOffset += UITheme::SPACING_SM;
    fonts.drawText("Press M to toggle J2", x, yOffset, UITheme::FONT_SIZE_SMALL,
                   UITheme::TEXT_MUTED);
    yOffset += 18;

    if (forceModel.j2Perturbation)
    {
        fonts.drawText("Orbit precession active", x, yOffset, UITheme::FONT_SIZE_SMALL,
                       UITheme::WARNING);
    }
}
